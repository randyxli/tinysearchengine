...
int getNextWord(int pos, char *html, char *nextword)
{
     if(pos == 0)
	  removeNonAlphas(html);
          
     while(!isalpha(html[pos]))
     {
	  if(!html[pos])
	       return -1;
	  pos++;
     ...
     strncpy(nextword, html+start, pos-start);
     return pos;
}
...
int NormalizeURL(char* URL) {
  ...
  if(strstr(URL, "javascript:") != NULL)//javascript in the URL is not ok
       return 0;
...
void removeTag(char *html, int pos)
{
     html[pos] = 'a';
     
     while(html[pos] && (html[pos] != 62))
     {
	  html[pos] = ' ';
	  pos++;
     }
...
void removeNonAlphas(char *html)
{
     for(int i=0; html[i]; i++)
     {
	  if(html[i] == 60)//if current character is '<'
	       removeTag(html, i);
	  else if(!isalpha(html[i]))//proceeds if html[i] is not alphabetic
	       	  html[i] = ' ';
	  else html[i] = tolower(html[i]);
     }
}
